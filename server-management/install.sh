...
# Einfaches Log-Helper (kannst Du durch Deine log_message ersetzen)
log_message() {
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*"
}

# Meldung beim Source‐Laden
log_message "🔧 Installationsserver IP ermittelt: $SERVER_IP"
...
echo "║ └────────────────────────────────────────────────────────────┘ ║"
echo "║ ┌────────────────────────────────────────────────────────────┐ ║"
echo "║ │              🆕 NEUE SCRIPTS AUTOMATISCH                   │ ║"
echo "║ └────────────────────────────────────────────────────────────┘ ║"
# dynamische Skriptanzeige New:
for f in /usr/local/bin/*.{sh,py}; do
  # nur anzeigen, wenn nicht bereits in SCRIPT_PATHS
  if ! printf '%s\n' "${SCRIPT_PATHS[@]}" | grep -Fxq "$f"; then
    FN=$(basename "$f")
    printf "echo \"║ │  🆕 %-55s │ ║\"\n" "$FN"
  fi
done
...
if [ -n "$SSH_CONNECTION" ]; then  # Prüft, ob eine SSH-Verbindung besteht
    ...
    if [ $? -eq 0 ]; then  # Überprüft, ob cat_hosts.sh erfolgreich abgeschlossen (Exit-Code 0)
        sudo /usr/local/bin/help_skripts.sh  # Führt show_help_skripts.sh aus
    else
        echo "❌ cat_hosts.sh fehlgeschlagen. show_help_skripts.sh wird nicht ausgeführt."
    fi
fi
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
EOF
